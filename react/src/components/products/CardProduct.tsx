...
import { Publication, Product } from "../../types/models";
...
const CardProduct = ({ prod }: any) => {
  const product = {
    ...
    description:
      "Sandias frescas plantadas al aire libre, sin conservantes ni pesticidas.",
    price: "3,49",
    quantity: 3,
    userId: "pepelozano",
  ...
  if (prod) {
      ...
      >
        <div className="w-full max-w-2xl bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
          ...
          <div className="px-4 pb-4">
            ...
            <div className="text-sm mb-4">{prod.description}</div>
    ...
    return (
      <div className="h-full min-h-full max-h-full flex-shrink-0 flex flex-col items-center overflow-hidden transition-transform duration-300 ease-in-out transform hover:scale-105">
        Loading...
      </div>
    );
  }
